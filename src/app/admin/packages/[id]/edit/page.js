...
import {
  ...
  Tag, DollarSign, Image, Palette, List, Info, Sparkles, Save,
} from 'lucide-react';
...
/* ── Styled label + input/select/textarea ──────────────────────────── */
const inputCls =
...
export default function EditPackagePage() {
  ...
  if (fetching) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        <div className="w-16 h-16 rounded-2xl gradient-brand flex items-center justify-center shadow-lg animate-pulse">
          <Loader2 size={28} className="animate-spin text-white" />
        </div>
        <p className="text-gray-400 text-sm font-medium">Loading package details…</p>
      </div>
    );
  }
  ...
  if (!form) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        ...
        <p className="text-red-600 font-semibold">{error || 'Package not found'}</p>
        <Link href="/admin/packages" className="text-brand text-sm font-medium hover:underline">
          ← Back to packages
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      >
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="text-2xl font-black text-white leading-tight truncate">{form.title || 'Edit Package'}</h1>
            <p className="text-white/40 text-xs font-mono mt-1 truncate">/{form.slug}</p>
          </div>

          <div
            className="hidden sm:flex flex-shrink-0 items-center gap-2 px-4 py-2 rounded-xl text-white/60 text-xs font-semibold"
      ...
      {/* ── Toast messages ──────────────────────────────────────────────── */}
      {success && (
        <div className="flex items-center gap-3 mb-6 p-4 rounded-2xl border text-emerald-700 font-semibold text-sm animate-fade-in"
      ...
      {error && (
        <div className="flex items-center gap-3 mb-6 p-4 rounded-2xl border text-red-700 text-sm animate-fade-in"
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <Section icon={Info} title="Basic Info" accent="#D62965">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              <Field label="Slug" hint="Auto-generated from title. Lowercase letters, numbers and hyphens only.">
        ...
        <Section icon={Tag} title="Badges" accent="#7C3AED">
            ...
            {BADGE_OPTIONS.map(b => {
              ...
              return (
                <button
                  ...
                  style={
                    active
                      ? { background: 'linear-gradient(135deg,#D62965,#7C3AED)', color: '#fff', border: 'transparent', boxShadow: '0 4px 12px rgba(214,41,101,0.35)' }
          ...
          {form.badges.length > 0 && (
            <p className="text-[11px] text-gray-400">{form.badges.length} badge{form.badges.length > 1 ? 's' : ''} selected</p>
          )}
        </Section>
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex-1 py-3 rounded-xl font-black text-sm text-white transition-all hover:opacity-90 active:scale-98 disabled:opacity-60 flex items-center justify-center gap-2"
            style={{
              background: 'linear-gradient(135deg, #D62965 0%, #7C3AED 100%)',
              boxShadow: loading ? 'none' : '0 4px 20px rgba(214,41,101,0.4)',
            }}
          >
            {loading ? (
